...
namespace Project2Tuason162032
{
    public partial class LoginForm : Form
    {
        ...
        public LoginForm(List<string> registeredusernames)
        {
            regnames = registeredusernames;
            InitializeComponent();
        }

        private void btnLogin_Click(object sender, EventArgs e)
        {
            if (regnames.Contains(loginName.ToUpper()))
            {
                DialogResult = DialogResult.OK;
            }
            else
            {
                MessageBox.Show("Person has not been registered.");
            }
        }
    }
}
